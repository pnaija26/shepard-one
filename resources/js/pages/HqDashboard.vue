<template>
  <div class="min-h-screen bg-canvas text-ink">
    ...
    <div class="lg:pl-60">
      <header class="sticky top-0 z-30 border-b border-line bg-white/95 backdrop-blur">
        <div class="flex min-h-18 items-center gap-3 px-4 sm:px-6 lg:px-8">
          ...
          <div class="min-w-0 flex-1">
            <p class="truncate text-sm font-semibold">HQ leadership</p>
            <p class="truncate text-xs text-muted">Church-wide KPIs and branch comparisons</p>
          </div>
          <button type="button" class="rounded-md border border-line px-3 py-2 text-xs font-semibold" :disabled="dashboardStore.loading" @click="applyFilters">Apply filters</button>
        </div>
      </header>

      <main class="px-4 py-6 pb-24 sm:px-6 lg:px-8 lg:py-8 lg:pb-8">
        <section class="mb-6 border-b border-line pb-6">
          <p class="mb-1 text-xs font-semibold uppercase tracking-wider text-brand">HQ leadership</p>
          <h1 class="font-serif text-3xl font-bold">Church-wide dashboard</h1>
          <p v-if="dashboardStore.period" class="mt-2 text-sm text-muted">
            Period {{ dashboardStore.period.from }} to {{ dashboardStore.period.to }}
            <span v-if="dashboardStore.dashboard?.scope" class="ml-2 rounded-full bg-canvas px-2 py-0.5 text-xs capitalize">{{ dashboardStore.dashboard.scope.replace('_', ' ') }}</span>
          </p>
        </section>
        ...
        <div class="mb-4 flex flex-wrap items-end gap-3">
          <div>
            <label class="text-sm font-medium" for="branch-filter">Branch filter</label>
            <input id="branch-filter" v-model="dashboardStore.branchFilter" type="number" min="1" placeholder="All branches" class="mt-1 block min-h-11 w-40 rounded-md border border-line px-3 py-2 text-sm" />
          </div>
          <div>
        ...
        <div v-if="dashboardStore.dashboard" class="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
          ...
          >
            ...
            <p v-if="dashboardStore.definitions[key]" class="mb-2 text-xs text-muted">{{ dashboardStore.definitions[key] }}</p>
            ...
            <button
              v-if="metric.drill_down && metric.state !== 'unauthorized' && key !== 'branch_performance'"
              type="button"
        ...
        <section v-if="dashboardStore.branchComparison?.state === 'ready'" class="mt-8 rounded-md border border-line bg-white p-5">
          <h2 class="mb-2 font-semibold">Branch comparison</h2>
          <p class="mb-4 text-xs text-muted">{{ dashboardStore.branchComparison.definition }}</p>
          <div class="overflow-x-auto">
            <table class="min-w-full text-left text-sm">
              <thead>
                <tr class="border-b border-line text-xs uppercase text-muted">
                  <th class="px-3 py-2">Branch</th>
                  <th v-for="metric in comparisonMetrics" :key="metric" class="px-3 py-2 capitalize">{{ metricLabel(metric) }}</th>
                </tr>
              </thead>
              <tbody>
                <tr v-for="row in dashboardStore.branchComparison.branches" :key="row.branch_id" class="border-b border-line/60">
                  <td class="px-3 py-2 font-medium">{{ row.branch_name }}</td>
                  <td v-for="metric in comparisonMetrics" :key="metric" class="px-3 py-2">
                    <span v-if="row.metrics[metric]?.suppressed" class="text-muted" title="Suppressed for privacy">—</span>
                    <span v-else-if="row.metrics[metric]?.state === 'unauthorized'" class="text-muted">N/A</span>
                    <span v-else>{{ formatComparisonValue(metric, row.metrics[metric]?.value) }}</span>
                  </td>
                </tr>
              </tbody>
            </table>
          </div>
          <button type="button" class="mt-4 rounded-md border border-line px-2 py-1 text-xs" :disabled="dashboardStore.drillLoading" @click="openDrillDown('branch_performance')">
        ...
        <section v-if="dashboardStore.drillDown" class="mt-6 rounded-md border border-line bg-white p-5" aria-live="polite">
          ...
          <p v-if="dashboardStore.drillDown.definition" class="mb-3 text-xs text-muted">{{ dashboardStore.drillDown.definition }}</p>
      ...
      <nav class="fixed inset-x-0 bottom-0 z-20 border-t border-line bg-white/95 px-2 py-2 backdrop-blur lg:hidden" aria-label="HQ leadership navigation">
        <div class="mx-auto grid max-w-lg grid-cols-4 gap-1 text-center text-[11px]">
          <a href="/hq-dashboard" class="rounded-md px-2 py-2 font-semibold text-brand" aria-current="page">HQ</a>
          <a href="/organizations" class="rounded-md px-2 py-2 text-muted hover:text-ink">Branches</a>
...
import { useHqDashboardStore } from '../stores/hqDashboard'

const drawerOpen = ref(false)
const dashboardStore = useHqDashboardStore()

const comparisonMetrics = ['members', 'growth', 'attendance', 'giving', 'care', 'welfare']
